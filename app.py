...
def userLogin():
    if request.method == 'POST':
        ...
        if user:
            ...
            return redirect(url_for('userRegister'))  # Redirect to login page if email is invalid
...
def vote():
    if request.method == 'POST':
        ...
        if candidate:
            ...
            # Get the path of the logged-in user's image from the session
            email = session.get('email')
            if email:
                user = Student.query.filter_by(email=email).first()
                if user:
                    img1_path = user.student_image  # Path of the logged-in user's image
                    img2_path = 'static/captured_image.jpg'  # Newly captured image
                    
                    # Verify the images
                    verification_result = verify(img1_path, img2_path)
                    ...
                    if verification_result:
                        ...
                        flash('Voting not allowed. Image verification failed.')
                        return redirect(url_for('index'))
                else:
                    flash('User not found.')
                    return redirect(url_for('userLogin'))
            else:
                flash('Voted Successfullly')
                return redirect(url_for('userLogout'))
        else:
